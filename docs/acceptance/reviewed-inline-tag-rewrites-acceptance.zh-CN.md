...
## 2026-09-10 整体 review 修复

修复陈旧缓存标签前缀误写、预检窗口并发写入、撤销后日志删除保存失败和 0.1.2 推荐日志加载四个问题。新增回归覆盖 Unicode/嵌套/转义边界、共享锁跨事务服务拒绝重入、持久化失败后的两类补偿和重试，以及插件升级撤销与已打开推荐弹窗的写入阻断。

本轮为自动化验证；既有 Obsidian 1.13.7 核心 smoke 是历史证据，本轮未重新执行真实宿主验收。最低支持版本及视觉验收未完成项继续保留。

验证结果：`npm test` 48 个文件、203 个用例通过；`npm run build` 通过；`npm run spec:validate -- --all --strict` 9/9 通过；`git diff --check` 通过。
