...
本技术方案面向第一版 Obsidian 插件实现。产品目标不是做一个普通的“AI 自动打标签”工具，而是做一个可解释、可预览、可撤销的 **AI 标签管理与治理插件**。

第一版应优先证明四件事：

- 能可靠扫描库中已有标签，并建立可查询的标签索引；
...
2. 库读取层：读取 Markdown 文件、frontmatter、inline tags 和当前编辑器内容。
...
产品级版本边界、MVP 完整版范围和后续迭代需求以 [路线图](roadmap.zh-CN.md) 为准。本节保留技术实施顺序，帮助把路线图拆成可开发模块。
...
交付标准：能在 Obsidian 中运行插件，并输出当前库的标签统计。
...
交付标准：用户能看到库的标签体系中的真实问题，但插件仍不自动批量改写。
...
- 插件能在 Obsidian 中加载；
- 能扫描库 并统计 frontmatter / inline tags；
