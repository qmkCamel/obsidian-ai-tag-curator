...
## 11. 整体 review 修复（2026-09-10）

- [x] 11.1 reader 与 writer 校验完整 token 边界，拒绝缓存中仍匹配较长标签前缀的区间，并覆盖嵌套、Unicode、转义与拼接场景。
- [x] 11.2 在异步预检前获取 OperationLog 共享互斥锁，覆盖清理、批次、推荐、撤销与恢复，验证重入拒绝和异常释放。
- [x] 11.3 日志删除持久化失败时恢复原记录与顺序，验证撤销补偿成功/失败、固定目标恢复及后续重试。
- [x] 11.4 兼容缺少来源字段的历史推荐记录，验证加载、深拷贝、序列化与真实插件编排下的撤销。
- [x] 11.5 运行新增回归、完整测试、构建与 OpenSpec 严格校验，记录本轮验证边界。
