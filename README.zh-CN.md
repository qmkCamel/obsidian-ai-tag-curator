...
面向 Obsidian vault 的 AI 标签治理与策展插件。

这个插件不是普通的“AI 标签生成器”，而是一个帮助用户维护标签体系的策展工具。它会优先理解和复用 vault 中已经存在的标签，而不是随意创建一批新标签。
...
2. 执行 `刷新标签索引`，扫描当前 vault 的标签体系。
3. 打开一篇 Markdown 笔记。
4. 执行 `为当前笔记推荐标签`。
5. 在预览弹窗中检查推荐标签、置信度和解释。
6. 只保留想要应用的标签，然后确认写入。
7. 如果需要撤销最近一次本插件对当前笔记的标签修改，执行 `撤销当前笔记最近标签修改`。
